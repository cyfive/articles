---
Title: Заметки по работе с SSH
Date: 2021-06-24 00:00
Modified: 2021-06-24 00:00
Category: Блог
Slug: ssh-notes
Authors: Емец Станислав
Summary: ![](/images/ssh-notes.png) За долгое время работы с SSH у меня накопилось немного заметок, которые позволяют улучшить качество работы ним.
---

# Заметки по работе с SSH

![](/images/ssh-notes.png)

За долгое время работы с SSH у меня накопилось немного заметок, которые позволяют улучшить качество работы с ним.

## Настройка alive

Часто при работе через разные VPN или нестабильные каналы связи у вас может возникать ситуация, что сессия подвисает, это лечится настройкой alive. С ним будет поддерживать соединение, а если оно разорвалось, вовремя его закрывать. Настраивается это в файле `~/.ssh/config` добавлением следующих настроек:

```
Host *
    ServerAliveInterval 600
```
Где 600 это время в секундах как часто отправлять alive пакеты. Я обычно использую значение 600 (10 минут), но если у вас нестабильный канал, или вы используете socks proxy, то значение нужно уменьшать.
...
- `127.0.0.1:2222` локальный интерфейс и порт на который будет проброшен удаленный порт.
